...
export default function AdminDashboard() {
  ...
  const sendWhatsApp = (order: Order) => {
    ...
    const message = `مرحباً ${order.customer_name}%0A%0Aتحديث طلبك رقم: ${order.order_number}%0Aالحالة: ${statusLabel}%0A%0Aمبلغ الشراء: ${Number(order.purchase_amount).toLocaleString()} ريال%0Aالمبلغ المستلم: ${Number(order.final_amount).toLocaleString()} ريال%0A%0Aشكراً لتعاملك مع liilsol`
