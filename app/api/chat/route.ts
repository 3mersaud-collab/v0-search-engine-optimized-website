...
function checkRateLimit(visitorId: string): boolean {
  ...
  if (!limit || now > limit.resetAt) {
    rateLimitMap.set(visitorId, { count: 1, resetAt: now + 60_000 }) // 1 minute window
    return true
  }
  if (limit.count >= 15) return false // Max 15 messages per minute
  limit.count++
  return true
}
...
export async function POST(req: Request) {
  try {
    ...
    // Extract user text before streaming
    ...
    if (userText) {
      saveMessage(visitorId, "user", userText).catch(() => {})
    }
...
// Save a message to Supabase conversation
async function saveMessage(visitorId: string, role: "user" | "assistant", content: string) {
  try {
    ...
    if (existing) {
      ...
      // Notify admin of new website conversation
      if (newConv) {
