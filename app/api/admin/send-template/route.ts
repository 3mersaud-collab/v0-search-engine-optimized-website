...
const TEMPLATES: Record<string, (data?: Record<string, string>) => string> = {
  order_ready: (data) =>
    `هلا ${data?.name || "يالغالي"}!\n\nطلبك رقم ${data?.orderNumber || ""} جاهز والمبلغ بيتحول لحسابك خلال ساعتين ان شاء الله.\n\nشكراً لثقتك بليل سول!`,
  ...
  welcome: () =>
    `هلا والله! انا مطر، مساعدك في ليل سول.\n\nنحول لك مشتريات التقسيط من تابي وتمارا ومدفوع لكاش بحسابك.\n\nالطريقة بسيطة: تحدد المبلغ > نحدد لك جهاز > تشتريه بالتقسيط > نستلمه ونحول لك الكاش خلال ساعتين.\n\nالدفعة الأولى ندفعها عنك - ما تدفع شي من جيبك!\n\nكم تحتاج كاش؟`,
}
